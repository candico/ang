component accessors="true" {
	...
	public any function onMissingMethod() {
        local.res = invoke(variables.evalDao, arguments.missingMethodName, arguments.missingMethodArguments);
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    
	...
	/*public function getUsersByEval(){
        return Variables.commonDao.QueryToArray( Variables.evalDao.getUsersByEvalQry(argumentcollection = arguments) );
    }*/

	public struct function getUsersByEval(){
    	var ls = {};
		ls.role_groups = {};
        
		ls.qry = Variables.evalDao.getUsersByEvalQry(argumentcollection = arguments);
		...
		for( ls.row in ls.qry ){        
            ...
            ls.user['extern_id'] = ls.row.extern_id;
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    
	...
	public function getUsersByRole(){
		...
		ls.qry = Variables.evalDao.getUsersByRole(argumentcollection = arguments);
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    
	...
	public function getEvalTypes(){
    	return Variables.commonDao.QueryToArray( Variables.evalDao.getEvalTypesQry(argumentcollection = arguments) );
    }

    public function getEvalData(){
    	return Variables.commonDao.QueryToArray( Variables.evalDao.getEvalDataQry(argumentcollection = arguments) );
    } 
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// function getEvalueeDetails
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    
	
	public function getEvalueeDetails(){
    	var ls = {};	
        
        if (arguments.id eq 0) return ls;
        
        arguments.user_id = request.session.user.user_id;
        arguments.office_id = request.session.user.settings.current_field_office_id;
        arguments.lang_code = request.session.user.settings.displayLanguage;
        arguments.eval_id = arguments.id;

        ls.data = {};     
        ls.data["full_name"] = "";
        ls.data["first_echo_contract_start"] = "";
        ls.data["current_role_id"] = 0;
        ls.data["current_role"] = "";
        ls.data["current_role_start"] = "";
        ls.data["current_contract_end"] = "";
        ls.data["home_office_id"] = "";
        ls.data["home_office"] = "";
        ls.data["staff_type"] = "";       
        
        ls.participants = getUsersByEval( argumentcollection = arguments );
      	ls.extern_id = ls.participants.evaluees[1].extern_id; //id in TBL_EXTERNS
        ls.evaluee_id = ls.participants.evaluees[1].user_id; //id in FSM_USERS
        ls.data["evaluee_id"] = ls.evaluee_id;        
		  		         
        //Note: rows are in chronological order (by contract id)
		ls.qry = Variables.evalDao.getEvalueeDetailsQry(user_id, office_id, ls.extern_id, lang_code); 
        for (ls.row in ls.qry){
        	ls.data.full_name = ls.row.last_name & " " & ls.row.first_name;
        	if(ls.data.first_echo_contract_start == "")
	            ls.data.first_echo_contract_start = ls.row.contract_start;
			if(ls.row.role_id != ls.data.current_role_id){
            	ls.data.current_role_id = ls.row.role_id;
                ls.data.current_role = ls.row.role;
                ls.data.current_role_start = ls.row.contract_start;
            }
            ls.data.current_contract_end = ls.row.contract_end;
            ls.data.staff_type = ls.row.staff_type;
            ls.data.home_office_id = ls.row.home_office_id;	
            ls.data.home_office = ls.row.home_office;	
        }       
