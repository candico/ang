...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---->
	...
	<cffunction name="getEvalGridDataQry" access="public" returntype="query">
		...
		<cfquery name="ls.qry" datasource="#Application.settings.dsn#">   
          ...
          (              
                ...
                WHERE
                    ...
                    AND US.ID =  <cfqueryparam value="#user_id#" cfsqltype="cf_sql_integer" />
                    AND RO.ROLE_CODE = STR.STR_CODE
                    AND STR.LANG_CODE = <cfqueryparam value="#lang_code#" cfsqltype="cf_sql_varchar" />  
            ...
            (
                SELECT
	                FBE.EVAL_ID EVAL_ID,
    	            LISTAGG(STR.STR || ' (> ' || NVL(TO_CHAR(FBE.END_ON,'DD/MM/YYYY'),'N/A') || ')', ', ') WITHIN GROUP (ORDER BY EF.ID) AS CURRENT_PHASES
                FROM
        	        FSM_EVAL_PHASES EF,
...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---->
	...
	<cffunction name="getUsersByEvalQry" access="public" returntype="query">
	    <cfargument name="user_id" type="numeric" required="true" />
        <cfargument name="office_id" type="numeric" required="true" />
        ...
        <cfquery name="ls.qry" datasource="#Application.settings.dsn#">
        	SELECT
                ...
                END ROLE_POS_ID 
			FROM
            	V_FSM_PARTICIPANTS_BY_EVAL PBE,
                ( 
                	SELECT 
...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---->
	...
	<cffunction name="getUsersByRoleQry" access="public" returntype="query">
	    <cfargument name="user_id" type="numeric" required="true" />
		<cfargument name="office_id" type="numeric" required="true" />   
        <cfargument name="eval_id" type="numeric" required="true" />    
        ...
        <cfquery name="ls.qry" datasource="#Application.settings.dsn#">   
            ...
            WHERE
                ...
                AND (RBE.ID IS NULL OR RBE.EVAL_ID = <cfqueryparam value="#eval_id#" cfsqltype="cf_sql_integer" />)
                AND RBU.USER_ID = US.ID
            ORDER BY
                LNAME,
                FNAME,                
...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---->
	...
	<cffunction name="getEvalDataQry" access="public" returntype="query">
	    <cfargument name="user_id" type="numeric" required="true" />
        <cfargument name="office_id" type="numeric" required="true" />
    	<cfargument name="eval_id" type="numeric" required="true" />
		<cfargument name="lang_code" type="string" required="true" /> 
        ...
        <cfquery name="ls.qry" datasource="#Application.settings.dsn#">  
            SELECT 
                EV.ID EVAL_ID,
                TO_CHAR(EV.START_ON,'DD/MM/YYYY') START_ON,
                TO_CHAR(EV.END_ON,'DD/MM/YYYY') END_ON,
                EV.TYPE_ID,
                EV.STATUS_ID,
                STA.STATUS_CODE                
            FROM
...
-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=---->
	...
	<cffunction name="getEvalTypesQry" access="public" returntype="query">
	    <cfargument name="user_id" type="numeric" required="true" />
    	<cfargument name="office_id" type="numeric" required="true" />
        <cfargument name="lang_code" type="string" required="true" />
